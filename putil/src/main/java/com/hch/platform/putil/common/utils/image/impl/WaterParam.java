...
public class WaterParam implements IWaterRunner {
    ...
    @Override
    public RstatusGroup exec() {
      RstatusGroup rtRsg = new RstatusGroup();
      RstatusGroup srcRsg = this.src.validate();
      RstatusGroup waterRsg = this.water.validate();
      RstatusGroup backIconRsg = this.backIcon.validate();
      RstatusGroup backTextRsg = this.backText.validate();

      RstatusGroup paramRsg = new RstatusGroup();
      if((param != null) && (param.size() > 0)){
        List<Rstatus> fails = Lists.newArrayList();
        List<Rstatus> successs = Lists.newArrayList();
        ...
        for (Water<?> rsgroup : param) {
          ...
          RstatusGroup curRsg = rsgroup.validate();
