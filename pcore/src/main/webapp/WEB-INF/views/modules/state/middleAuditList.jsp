...
<div class="container-fluid container-fluid-oe">
    ...
    <form:form id="searchForm" modelAttribute="act" action="${ctx}/state/middleAuditList" method="post"
    ...
    <table id="contentTable" class="table table-bordered table-condensed table-hover table-theme-default table-center">
        ...
        <c:forEach items="${page.list}" var="act">
            <tr>
                <td>
                        <p class="team-number">${act.status == "other"? act.projectDeclare.number :act.vars.map.number}</p> <%--${act.vars.map.number}--%>
                </td>
                <td>
                    <c:choose>
                        <c:when test="${act.status=='other'}">
                            <a href="${ctx}/state/projectDetail?id=${act.projectDeclare.id}">
                               ${act.projectDeclare.name}
                           </a>
                        </c:when>
                        <c:otherwise>
                            <p class="team-name">
                                   <a href="${ctx}/state/infoView?id=${act.vars.map.id}&taskDefinitionKey=${act.histTask.taskDefinitionKey}">
                                   ${act.vars.map.name}</a>
                               </p>
                        </c:otherwise>
                    </c:choose>
                ...
                <td>
                        ${pj:getTeamNum(act.status == "other"? act.projectDeclare.snames :act.vars.map.teamList)}
                </td>
                <td>
                ...
                <td style="white-space: nowrap">
                    ...
                    <c:if test="${act.status=='todo'}">
                        <a class="btn btn-back-oe btn-primaryBack-oe btn-small"
                           href="${ctx}/act/task/form?taskId=${act.task.id}&taskName=${fns:urlEncode(act.task.name)}&taskDefKey=${act.task.taskDefinitionKey}&procInsId=${act.task.processInstanceId}&procDefId=${act.task.processDefinitionId}&status=${act.status}">评分</a>
                    </c:if>
                    <c:if test="${act.status=='finish'}">
                        <a class="btn btn-back-oe btn-primaryBack-oe btn-small"
