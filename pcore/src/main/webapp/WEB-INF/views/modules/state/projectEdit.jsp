...
<body>
			...
			<div class="container-fluid content-wrap">
				...
				<div class="row">
					...
					<div class="form-horizontal">
						...
						<div class="form-group col-sm-4 col-md-4 col-lg-4">
							...
							<p class="col-xs-7" id="zylj">${fns:getOffice(leader.professional).name}<fmt:formatDate
						...
						<div class="form-horizontal row my-form-horizontal" novalidate>
							...
							<div>
								...
								<span class="tips-wrap"> <input type="text" name="name"
									...
									class="my-textarea required" style="width: 567px;" />
						...
						<div class="form-horizontal row prj-plan my-form-horizontal"
							...
							<label><span class="star">*</span>项目拓展及传承规划：</label> <span
								class="tips-wrap"> <form:checkboxes path="development"
									items="${fns:getDictList('project_extend')}" itemLabel="label"
									itemValue="value" class="required" /> <input type="hidden"
				...
				<section class="row">
					<div class="prj_common_info">
						<h3>团队信息</h3>
						<span class="yw-line"></span> <a href="javascript:;"
							id="teamToggle" data-flag="true"><span
							class="icon-double-angle-up"></span></a>
					</div>
					<div class="toggle_wrap" id="teamToggle_wrap">
						<table class="table  table-hover table-condensed table-bordered studenttb">
							<caption>
								<strong
									style="font-weight: 500; font-size: 14px; margin-left: -15px; color: #656565;"><span
									class="star">*</span>项目团队：</strong><span>${team.name}</span><span class="star xfpb">学分配比规则：</span><span id="xfpbgz" class="star xfpb"></span>
							</caption>
							...
							<thead>
								<tr>
									...
									<th><input type="button" class="btn" userType="1" value="新增"
							...
							<tbody>
								<c:forEach items="${turStudents}" var="item" varStatus="status">
									<tr userid="${item.userId}">
										<input class="custindex" type="hidden" name="studentList[${status.index}].userId" value="${item.userId}">
										<input class="custindex" type="hidden"  name="studentList[${status.index}].utype" value="1">
										<td>${item.name}</td>
										<td>${item.no}</td>
										<td>${item.mobile}</td>
										<td>${item.org_name}</td>
	                                    ...
	                                    <td>
	                                        <select class="zzsel custindex"  name="studentList[${status.index}].userzz">
	                                            <option value="0" <c:if test="${item.userId==projectDeclare.leader}">selected</c:if> >
	                                                	负责人
	                                            </option>
	                                            <option value="1"  <c:if test="${item.userId!=projectDeclare.leader}">selected</c:if> >
	                                            		组成员
	                                            </option>
	                                        </select>
	                                    </td>
	                                    <td class="xfpb">
											<input type="text" class="custindex xfpbval" style="width: 30px;" maxlength="10" name="studentList[${status.index}].weightVal"
						...
						<table class="table  table-hover table-condensed table-bordered teachertb">
							<caption class="prj_tab_caption"
								style="font-size: 14px; color: #656565;">指导老师</caption>
							<thead>
								<tr>
									<th>姓名</th>
									<th>工号</th>
									...
									<th>联系电话</th>
									<th>当前指导</th>
									<th><input type="button" class="btn" userType="2" value="新增"
                                       onclick="selectUser(this)"/></th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${turTeachers}" var="item" varStatus="status">
									<tr userid="${item.userId}">
										<input type="hidden" name="teacherList[${status.index}].userId"
                                           value="${item.userId}">
                                    	<input type="hidden" name="teacherList[${status.index}].utype" value="2">
										<td>${item.name}</td>
										<td>${item.no}</td>
					...
					<div class="toggle_wrap" id="projectToggle_wrap">
						<div class="prj_introduce">
							...
							<span class="tips-wrap"> <textarea
									class="prj_desc required" name="introduction">${projectDeclare.introduction}</textarea>
								<p class="name-err-tips"></p>
							</span>
						</div>
						...
						<div class="prj_introduce">
							...
							<span class="tips-wrap"> <textarea
									class="prj_desc required" name="innovation">${projectDeclare.innovation}</textarea>
								<p class="name-err-tips"></p>
							</span>
						</div>
								...
								<tbody>
									<tr>
										<td class="tips-wrap"><textarea id="planContent"
												name="planContent" class="my-textarea required">${projectDeclare.planContent}</textarea>
										...
										<td class="tips-wrap"><textarea id="planStep"
												name="planStep" class="my-textarea required">${projectDeclare.planStep}</textarea>
											<p class="tname-err-tips"></p></td>
									</tr>
								</tbody>
							</table>
					...
					<div class="toggle_wrap" id="resultToggle_wrap">
						<form class="form-horizontal" novalidate>
							...
							<div class="form-group col-sm-12 col-md-12 col-lg-12 prj-plan"
								...
								<label style="width: 100px; margin-left: -15px;"><span
									...
									class="tips-wrap"> <form:checkboxes path="resultType"
										...
										itemLabel="label" itemValue="value" class="required" /> <input
							...
							<div class="form-group col-sm-12 col-md-12 col-lg-12"
								...
								<textarea name="resultContent" maxlength="512"
				...
				<section class="row">
					...
					<div class="budget" id="budgetToggle_wrap">
						<textarea name="budget" maxlength="512" class="change-budget">${projectDeclare.budget}</textarea>
					</div>
				</section>
				...
				<section class="row">
					<div class="prj_common_info">
						<h3>审核记录</h3>
						<span class="yw-line"></span> <a href="javascript:;"
							id="checkRecord" data-flag="true"><span
							class="icon-double-angle-up"></span></a>
					</div>
					<div style="padding: 10px" class="toggle_wrap checkRecord_wrap"
						id="checkRecord_wrap">
						<table class="table table-hover table-bordered">
							...
							<thead class="th_align-center">
								<th width="20%" class="m-table-th-h">审核人</th>
								<th width="10%" class="m-table-th-h">项目评级</th>
								<th width="50%" class="m-table-th-h">建议及意见</th>
								<th width="20%" class="m-table-th-h">审核时间</th>
							</thead>
							<tbody>

								<tr>
									<input type="hidden" name="auditInfoList[0].createBy.id"
										value="${info01.createBy.id}" />
									<input type="hidden" name="auditInfoList[0].createDate"
										value="<fmt:formatDate value='${info01.createDate}' pattern='yyyy-MM-dd HH:mm:ss'/>" />
									<input type="hidden" name="auditInfoList[0].auditStep"
										value="1" />

									<td align="center">${info01.userName}</td>
									<td align="center" class="tips-wrap "><select
										id="info01Grade" name="auditInfoList[0].grade"
										class="infoMsg required">
											<option value="">--请选择--</option>
											<option value="6">提交给校级评审</option>
											<option value="4">C</option>
											<option value="5">不合格</option>
									</select> <span class="name-err-tips prj-record-tips"></span></td>
									<td><textarea name="auditInfoList[0].suggest"
											class="my-textarea" maxlength="500">${info01.suggest}</textarea>
									</td>
									<td align="center"><fmt:formatDate
											value="${info01.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />
									</td>
								</tr>

								<tr>
									<input type="hidden" name="auditInfoList[1].createBy.id"
										value="${info02.createBy.id}" />
									<input type="hidden" name="auditInfoList[1].createDate"
										value='<fmt:formatDate value="${info02.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>' />
									<input type="hidden" name="auditInfoList[1].auditStep"
										value="1" />
									<td align="center">${info02.userName}</td>
									<td align="center" class="tips-wrap"><select
										id="info02Grade" name="auditInfoList[1].grade"
										class="infoMsg required">
											<option value="">--请选择--</option>
									...
									<td><textarea name="auditInfoList[1].suggest"
											class="my-textarea" maxlength="500">${info02.suggest}</textarea>
									</td>
									<td align="center"><fmt:formatDate
											value="${info02.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />
									</td>
								</tr>
							...
							<thead class="th_align-center">
								<th class="m-table-th-h">评分专家</th>
								<th class="m-table-th-h">评分</th>
								<th class="m-table-th-h">建议及意见</th>
								<th class="m-table-th-h">审核时间</th>
							</thead>
							<tbody id="tbodyInfo2">
							</tbody>
							<tbody>
								<tr>
									...
									<input type="hidden" id="midScore" name="midScore"
										value="${projectDeclare.midScore}" />
									<td colspan="3" class="result_score" id="midScoreTd">
										${projectDeclare.midScore}</td>
								</tr>
							</tbody>
							...
							<thead class="th_align-center">
								<th class="m-table-th-h">审核人</th>
								<th class="m-table-th-h">中期结果</th>
								<th class="m-table-th-h">建议及意见</th>
								<th class="m-table-th-h">审核时间</th>
							</thead>
							<tbody id="tbodyInfo3">
								<tr>
									<input type="hidden" name="auditInfoList[2].createBy.id"
										id="info3Mid" value="${info3.userId}" />
									<input type="hidden" name="auditInfoList[2].createDate"
										value='<fmt:formatDate value="${info3.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>' />
									<input type="hidden" name="auditInfoList[2].auditStep"
										value="3" />
									<td align="center" id="info3Name">${info3.userName}</td>
									<td align="center" class="tips-wrap"><select
										name="auditInfoList[2].grade" id="info3Grade"
										class="infoMsg required">
											<option value="">--请选择--</option>
											<option value="0">合格</option>
											<option value="2">项目终止</option>
									</select> <span class="name-err-tips prj-record-tips"></span></td>
									<td><textarea name="auditInfoList[2].suggest"
											class="my-textarea" maxlength="500">${info3.suggest}</textarea>
									</td>
									<td align="center"><fmt:formatDate
											value="${info3.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />
									</td>
								</tr>
							</tbody>
							...
							<tbody>
								<tr>
									...
									<input type="hidden" id="finalScore" name="finalScore"
										value="${projectDeclare.finalScore}" />
									<td colspan="3" class="result_score" id="finalScoreTd">${projectDeclare.finalScore}</td>
								</tr>
							</tbody>
							...
							<tbody id="tbodyInfo5">
								<tr>
									<input type="hidden" name="auditInfoList[3].createBy.id"
										id="info5Reply" value="${info5.userId}" />
									<input type="hidden" name="auditInfoList[3].createDate"
										value='<fmt:formatDate value="${info5.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>' />
									<input type="hidden" name="auditInfoList[3].auditStep"
										value="5" />
									<td align="center" id="info5Name">${info5.userName}</td>
									<td align="center" class="tips-wrap"><input
										name="auditInfoList[3].score" class="required number infoMsg"
										value="<fmt:formatNumber type='number' value='${info5.score}' maxFractionDigits='0'/>" />
										<span class="name-err-tips prj-record-tips"></span></td>
									<td><textarea name="auditInfoList[3].suggest"
											class="my-textarea" maxlength="500">${info5.suggest}</textarea>
									</td>
									<td align="center"><fmt:formatDate
											value="${info5.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />
									</td>
								</tr>
							</tbody>
							...
							<thead class="th_align-center">
								<th class="m-table-th-h">审核人</th>
								<th class="m-table-th-h">结果</th>
								<th class="m-table-th-h">建议及意见</th>
								<th class="m-table-th-h">审核时间</th>
							</thead>
							<tbody>
								<tr>
									<input type="hidden" name="auditInfoList[4].createBy.id"
										id="info6Assess" value="${info6.userId}" />
									<input type="hidden" name="auditInfoList[4].createDate"
										value='<fmt:formatDate value="${info6.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>' />
									<input type="hidden" name="auditInfoList[4].auditStep"
										value="6" />
									<td align="center" id="info6Name">${info6.userName}</td>
									<td align="center" class="tips-wrap"><select
										name="auditInfoList[4].grade" id="info6Grade"
										class="infoMsg required">
											<option value="">--请选择--</option>
											<option value="0">合格</option>
											<option value="1">优秀</option>
											<option value="2">不合格</option>
											<option value="5">延期结项</option>
									</select> <span class="name-err-tips prj-record-tips"></span></td>
									<td><textarea name="auditInfoList[4].suggest"
											class="my-textarea" maxlength="500">${info6.suggest}</textarea>
									</td>
									<td align="center"><fmt:formatDate
											value="${info6.createDate}" pattern="yyyy-MM-dd HH:mm:ss" />
									</td>
								</tr>
				...
				<section class="row">
					...
					<div class="row" style="text-align: center;">
						...
						<div class="btn-upload-file btn-upload-file-small" id="upload">上传附件</div>
						<input type="file" style="display: none" id="fileToUpload"
							name="fileName" />
						...
						<a href="javascript:;" class="btn btn-sm btn-primary" role="btn"
							onclick="submit();" id="submitBtn">保存</a> <a href="javascript:;"
	...
	<tr>
		...
		<td class="td_input tips-wrap">
			<textarea  name="planList[{{nameIdx}}].content"   class="my-textarea required planListTextarea">{{row.content}}</textarea>
			<span class="tname-err-tips"></span>
		</td>
		<td class="td_input tips-wrap">
			<textarea  name="planList[{{nameIdx}}].description"  class="my-textarea required planListTextarea">{{row.description}}</textarea>
			<span class="tname-err-tips"></span>
		</td>

		<td class="tips-wrap">
			<input  id="startDate[{{idIdx}}]"   name="planList[{{nameIdx}}].startDate" class="Wdate required planListStart" type="text"
					onClick="WdatePicker({maxDate:'#F{$dp.$D(\'endDate[{{idIdx}}]\')}'} )"
					value="{{row.startDate}}" />
			<span>至</span>
			<input  id="endDate[{{idIdx}}]"  name="planList[{{nameIdx}}].endDate" class="Wdate required planListEnd" type="text"
		...
		<td class="td_input tips-wrap">
			<input style="width:65px;"  name="planList[{{nameIdx}}].cost" class="my-textarea required planListInput number"   value="{{row.cost}}"/>
			<span class="tname-err-tips" style="left: 13px;"></span>
		</td>
		<td class="td_input tips-wrap">
			<input style="width:65px;"  name="planList[{{nameIdx}}].quality"  class="my-textarea required planListInput"  value="{{row.quality}}" />
			<span class="tname-err-tips"></span>
		</td>


		<td class="opt_btn" style="width:80px">
			<a href="javascript:;"  onclick="addRow('#tbody1',tpl,'');" class="btn btn-sm btn-primary btn-sm-reset btn-add"><span class="icon-plus"></span>添加</a>
	...
	<tr>
		...
		<td align="center" class="tips-wrap">
			<input name="midAuditList[{{idx}}].score" value="{{row.score}}"  class="midScore required number infoMsg " onblur="delMidScore();" />
			<span class="name-err-tips prj-record-tips"></span>
		</td>
		<td>
			<textarea name="midAuditList[{{idx}}].suggest"  class="my-textarea" maxlength="500" >{{row.suggest}}</textarea>
		</td>
		<td align="center">
			{{row.createDate}}
		</td>
	...
	<script type="text/template" id="tplInfo4">
		<tr>
			...
			<td align="center" class="tips-wrap">
				<input name="closeAuditList[{{idx}}].score" value="{{row.score}}"  class="closeScore required number infoMsg " onblur="delCloseScore();" />
				<span class="name-err-tips prj-record-tips"></span>
			</td>
			<td>
				<textarea name="closeAuditList[{{idx}}].suggest" class="my-textarea" maxlength="500" >{{row.suggest}}</textarea>
			</td>
			<td align="center">
				{{row.createDate}}
			</td>
...
<script type="text/template" id="tpl_st">
    <tr userid="{{userId}}">
		...
		<td class="xfpb">
			<input type="text" class="custindex xfpbval" style="width: 30px;" maxlength="10" name="studentList[custindex].weightVal"
			value="${item.weightVal}">
		</td>
        <td>
            <a class="minus"></a>
