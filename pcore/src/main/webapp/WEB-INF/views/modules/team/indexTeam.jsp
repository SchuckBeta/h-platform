        ...
        function showForm() {
            ...
            if (checkUserInfoData.ret==1) {
                ...
                if(checkUserInfoData.ret==2){
                	showModalMessage(0, checkUserInfoData.msg, {
		                        	确定: function () {
				...
				<div class="top-content" style="padding-bottom: 20px;">
					...
					<div class="form-group">
						<div class="col-xs-12  text-right">
							<button id="btnSubmit" class="btn btn-primary-oe" type="submit"
			...
			<div class="form-search-block clearfix">
				...
				<div class="pull-right">
					<button type="button" class="btn btn-primary"
						onclick="page(${page.pageNo},${page.pageSize});">查询</button>
				</div>
				<div class="form-inline">
					...
					<div class="form-group">
						<label class="control-label">团队负责人：</label> <input type="text"
							id="creator" class="form-control" value="${qryForm.creator}" />
					</div>
					<div class="form-group">
					...
					<c:forEach items="${page.list}" var="team" varStatus="status">
						<tr>
							...
							<td><c:if
								...
								<c:if
									test="${curUserId == team.sponsorId||(team.state!=3&&team.state!=4)}">${team.name}</c:if>
							</td>
							...
							<td><c:if
								...
								<c:if test="${curUserId != team.sponsorId&&team.state==4}">信息审核未通过</c:if>
								<c:if
									test="${curUserId == team.sponsorId||(team.state!=3&&team.state!=4)}">${fns:abbr(team.projectIntroduction,30)}</c:if>
							</td>
							<td><span
								class="<c:if test="${team.userCount!=team.memberNum}" >red</c:if>">${team.userCount}</span>/<span>${team.memberNum}</span>
								<p class="team-member">${team.userName}</p></td>
							<td><span
								class="<c:if test="${team.schoolNum!=team.schoolTeacherNum}" >red</c:if>">
									${team.schoolNum} </span>/${team.schoolTeacherNum} </br>
								<p class="team-member">${team.schName }</p></td>
							</td>
							<td><span
								class="<c:if test="${team.enterpriseNum!=team.enterpriseTeacherNum}">red</c:if>">
							...
							<td class="my-btn-group">
								<div class="btn-group">
									...
									<ul class="dropdown-menu">
										<c:if
											test="${curUserId!=null && curUserId!=''
								&& team.sponsorId!=null && team.sponsorId!=''
								&& curUserId == team.sponsorId && team.state!=2}">
											<c:if test="${team.state==0 }">
												<li style="cursor: pointer;" data-toggle="modal"
													data-target="#myModal"><a data-id='${team.id}'>发布</a></li>
											</c:if>
											...
											<c:if test="${team.state==0||team.state==1||team.state==4}">
												<li><a href="javascript: void(0);" class="edit-team"
													data-id="${team.id}">编辑团队信息</a></li>
											</c:if>
										</c:if>
										<li><a href="${ctxFront}/team/findByTeamId?id=${team.id}">查看</a></li>
										<c:if
											test="${not team.checkJoin&&userType != 2 && team.state==0}">
											<li><a href="#"
												onclick="return checkIfLogin('${team.id}')">申请加入</a></li>
										</c:if>
										<c:if test="${not team.checkJoinTUR||team.state == 2}">
											<li><a href="javascript: void(0);" class="delete-team"
												data-id="${team.id}">删除</a></li>
										</c:if>
									</ul>
								</div>
							</td>
