...
<div class="container-fluid container-fluid-oe">
    ...
    <form:form id="searchForm" action="${ctx}/impdata/mdlist" method="post"
               class="form-top-search" cssStyle="overflow: hidden">
        ...
        <div class="search-form-wrap form-inline">
            <input class="btn btn-back-oe btn-primaryBack-oe" onclick="goback();" type="button" value="返回"/>
            <input id="import" class="btn btn-back-oe btn-primaryBack-oe" type="button" value="选择文件"/>
                    ...
                    <td style="white-space: nowrap">
                        <c:if test="${data.is_complete!=1}">
                            <a style="display: none" href="${ctx}/impdata/expData?id=${data.id}&type=${data.imp_tpye}"
                               class="btn isComplete_down btn-back-oe btn-primaryBack-oe btn-small">下载错误数据</a>
                            <a style="display: none" href="${ctx}/impdata/delete?id=${data.id}&referrer=${encodereferrer}"
                               class="btn isComplete btn-small"
                        ...
                        <c:if test="${data.is_complete==1}">
                            ...
                            <a href="${ctx}/impdata/expData?id=${data.id}&type=${data.imp_tpye}"
                               class="btn btn-back-oe btn-primaryBack-oe btn-small">下载错误数据</a>
                               </c:if>
                            <a href="${ctx}/impdata/delete?id=${data.id}&referrer=${encodereferrer}" class="btn btn-small"
                               onclick="return confirmx('确认要删除吗？', this.href)">删除</a>
                        </c:if>
                    </td>
                </shiro:hasPermission>
