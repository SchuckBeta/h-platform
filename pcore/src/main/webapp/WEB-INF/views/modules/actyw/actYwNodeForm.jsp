...
<body>
	...
	<div class="content_panel">
		...
		<form:form id="inputForm" modelAttribute="actYwNode" action="${ctx}/actyw/actYwNode/save" method="post" class="form-horizontal">
			...
			<div class="control-group">
				<label class="control-label">节点等级：</label>
				<div class="controls">
		   ...
		   <div class="control-group">
			...
			<div id="formIdDiv" class="control-group">
				<label class="control-label">默认表单：</label>
				<div class="controls">
					<form:select path="formId" class="input-xlarge required">
						<form:option value="" label="--请选择--"/>
						<c:forEach var="item" items="${actYwForms }">
							<c:if test="${item.id eq actYwNode.formId}">
								<option value="${item.id}" selected="selected">${fns:getDictLabel(actYwNode.type, 'act_form_type', '')}-${item.name}</option>
							</c:if>
							<c:if test="${item.id ne actYwGnode.formId}">
								<option value="${item.id}" >${fns:getDictLabel(actYwNode.type, 'act_form_type', '')}-${item.name}</option>
							</c:if>
						</c:forEach>
					</form:select>
				</div>
