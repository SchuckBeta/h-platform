...
<div class="container-fluid container-fluid-oe">
    ...
    <form:form id="searchForm" action="${ctx}/impdata/list" method="post"
        ...
        <div class="search-form-wrap form-inline">
            <input id="loadTemp" class="btn btn-back-oe btn-primaryBack-oe" type="button" value="下载模板"
                   onclick="downTemplate()"/>
            <input id="import" class="btn btn-back-oe btn-primaryBack-oe" type="button" value="导入"/>
            ...
            <div class="condition-row">
                <div class="condition-item">
                    <div class="control-group">
                        <label for="tempType" class="control-label">模板类型</label>
                        <div class="controls">
                            <select id="tempType" class="input-medium form-control">
    ...
    <%--<div class="table-responsive">--%>
        <table id="contentTable"
               class="table table-hover table-bordered table-condensed table-center table-theme-default">
            <thead>
            <tr>
                <th>导入数据类型</th>
                <th>开始导入时间</th>
                    ...
                    <td style="white-space: nowrap">
                        <c:if test="${data.is_complete!=1}">
                            <a style="display: none" href="${ctx}/impdata/expData?id=${data.id}&type=${data.imp_tpye}"
                               class="btn isComplete btn-back-oe btn-primaryBack-oe btn-small">下载错误数据</a>
                            <a style="display: none" href="${ctx}/impdata/delete?id=${data.id}" class="btn isComplete btn-small"
                               onclick="return confirmx('确认要删除吗？', this.href)">删除</a>
                        </c:if>
                        <c:if test="${data.is_complete==1}">
                            <a href="${ctx}/impdata/expData?id=${data.id}&type=${data.imp_tpye}" class="btn btn-back-oe btn-primaryBack-oe btn-small">下载错误数据</a>
                            <a href="${ctx}/impdata/delete?id=${data.id}" class="btn btn-small"
                               onclick="return confirmx('确认要删除吗？', this.href)">删除</a>
                        </c:if>
                    </td>
                </shiro:hasPermission>
