...
<div class="container-fluid">
    ...
    <ul class="nav nav-tabs nav-tabs-default">
        <li class="active"><a href="${ctx}/pw/pwEnter/list">入驻申报列表</a></li>
    </ul>
    <div class="tab-content-default">
        ...
        <table id="contentTable"
            ...
            <c:forEach items="${page.list}" var="pwEnter" varStatus="idx">
                <c:if test="${(empty pwEnter.eteam) && (empty pwEnter.eproject) && (empty pwEnter.ecompany) || (pwEnter.isTemp eq '1')}"><tr style="background-color: #eee;"></c:if>
                <c:if test="${((not empty pwEnter.eteam) || (not empty pwEnter.eproject) || (not empty pwEnter.ecompany)) && (pwEnter.isTemp eq '0')}"><tr></c:if>
                ...
                <shiro:hasPermission name="pw:pwEnter:edit">
                    <td style="white-space: nowrap">
                        <c:if test="${(empty pwEnter.eteam) && (empty pwEnter.eproject) && (empty pwEnter.ecompany) || (pwEnter.isTemp eq '1')}">
                        ...
                        <c:if test="${((not empty pwEnter.eteam) || (not empty pwEnter.eproject) || (not empty pwEnter.ecompany)) && (pwEnter.isTemp eq '0')}">
                            ...
                            <c:if test="${pwEnter.status eq 0}">
                                <a class="btn-primary btn-small btn"
                                   href="${ctx}/pw/pwEnter/form?id=${pwEnter.id}&isView=false">审核</a>
                            </c:if>
                        </c:if>
                    </td>
                </shiro:hasPermission>
