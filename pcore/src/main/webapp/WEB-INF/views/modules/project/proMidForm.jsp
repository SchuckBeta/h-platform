							...
							<div class="row content-wrap">
										...
										<table class="table  table-hover table-bordered">
											...
											<c:if test="${proMid.id!=null}">
												<c:forEach items="${proSituationList}" var="proSituation" varStatus="status">
													<tr>
														...
														<td class="td_input">
															<textarea name="proSituationList[${status.index}].division" required maxlength="2000">${proSituation.division}</textarea>
															<br/><span class="prj-division"></span>
														</td>
														<td class="td_input">
															<textarea name="proSituationList[${status.index}].situation" required maxlength="2000">${proSituation.situation}</textarea>
															<br/><span class="prj-division"></span>
														</td>
													</tr>
												</c:forEach>
											...
											<c:if test="${proMid.id ==null}">
												<c:forEach items="${turStudents}" var="tur" varStatus="status">
													<tr>
														<input type="hidden"  name="proSituationList[${status.index}].user.id" value="${tur.userId}" />
														<td>${tur.name}</td>
														<td class="td_input">
															<textarea name="proSituationList[${status.index}].division" required maxlength="2000"></textarea>
															<br/><span class="prj-division"></span>
														</td>
														<td class="td_input">
															<textarea name="proSituationList[${status.index}].situation" required maxlength="2000"></textarea>
															<br/><span class="prj-division"></span>
														</td>
														<input type="hidden" name="teamUserHistoryList[${status.index}].id" value="${tur.id}">
														<c:if test="${fns:checkMenuByNum(5)}">
														<td class="credit-ratio">
															<input class="form-control input-sm " name="teamUserHistoryList[${status.index}].weightVal" value="${tur.weightVal}">
														</td>
														</c:if>
													</tr>
												</c:forEach>
										...
										<table class="table  table-hover table-bordered">
											...
											<script type="text/template" id="tpl">
												<tr>
													...
													<td class="td_input">
														<textarea  name="proProgresseList[{{nameIdx}}].situation" required maxlength="2000">{{row.situation}}</textarea>
								...
								<section class="row">
									...
									<div class="textarea-wrap">
										<textarea name="programme" maxlength="2000" class="col-xs-12 col-md-12 col-sm-12 col-lg-12 my-textarea pro-textarea" required>${proMid.programme}</textarea>
								...
								<section class="row">
									...
									<div class="textarea-wrap">
										<sys:frontFileUpload fileitems="${fileListMap}" filepath="proMidForm" btnid="upload"></sys:frontFileUpload>
									</div>
								</section>
