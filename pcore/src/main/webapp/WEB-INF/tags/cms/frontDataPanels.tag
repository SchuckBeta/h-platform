		...
		<c:forEach var="resource" items="${region.childResourceList}">
			<div class="card sc-dyn-news">
				...
				<c:forEach var="o" items="${oaNotifys}">
					<li><span class="date"><fmt:formatDate value="${o.updateDate }" pattern="yyyy-MM-dd" /></span><a href="${ctxFront}/oa/oaNotify/viewDynamic?id=${o.id}" class="link" title="${o.title }">${fns:abbr(o.title,50)}</a></li>
				...
				<div class="more">
					<a href="${ctxFront}/page-SCtognzhi#${type}">更多...</a>
					<%-- <a href="${ctxFront}/oa/oaNotify/list?type=${type}">更多...</a> --%>
				</div>
			</div>
		</c:forEach>