...
+function ($) {
    var uploader = Vue.component('uploader', {
        template: '<div><div class="accessories-container"> ' +
        ...
        '<a :href="item.url" @click="downFile(item.url,  $event)"><img :src="item.type | imageType">' +
        ...
        '<div class="btn-accessory-box"> ' +
        '<div v-upload-file class="upload-content">上传附件</div> </div></div>',
        model: {
            prop: 'fileList',
            event: 'change'
        },
        ...
        methods: {
            ...
            downFile: function (item, $event) {
                ...
                location.href = "/ftp/ueditorUpload/downFile?url=" + (item.url || item.datas.url) + "&fileName=" + encodeURI(encodeURI((item.title || item.name) || (item.datas.title || item.datas.name)));
            },
            //删除附件
            removeAccessory: function (item, index) {
                ...
                dialogCyjd.createDialog(0, '是否删除附件' + (item.title || item.name), {
                    ...
                    buttons: [{
                        ...
                        click: function () {
                            var $this = $(this);
                            var xhr = $.post('/f/attachment/sysAttachment/ajaxDelete/' + (item.id || item.datas.id), {
                                ftpUrl: item.url
                            });
