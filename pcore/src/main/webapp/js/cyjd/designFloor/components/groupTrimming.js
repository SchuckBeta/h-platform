var groupTrimming = Vue.component('group-trimming', {
    template: '<div class="group" :class="{groupOpened: open}"><template v-if="trimming.shapeType === image">' +
    ...
    '<h4 class="group-label"><i class="iconfont iconfont-open" @click="open = true"></i><i class="iconfont iconfont-close" @click="open = false"></i><span>形状</span>' +
    '<a href="javascript:void(0);" @click="deleteEle($event)"><i class="iconfont iconfont-delete">&#xe625;</i></a></h4>' +
    ...
    '@click="fillBackgroundColor($event)"><img v-if="trimming.stroke === transparent" src="/images/transparent-icon.png" class="select-box-option-icon"></div></div></div></div></div>' +
