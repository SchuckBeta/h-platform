...
!function($){
    ...
    LessonCodeSearch.prototype.choose = function(){
        ...
        var self = this;
        $resultContainerId.off('.xz').on('click.xz','.btn-primary-oe',function(e){
        ...
        $resultModalContainerId.off('.xz').on('click.xz','.btn-primary-oe',function(e){
            var item = $(this).data('row');
    ...
    LessonCodeSearch.prototype.resultTemplate = function(data){
        ...
        $.each(list,function(i,item){
            trs += ' <tr><td>'+item.code +'</td><td>'+item.name +'</td><td>'+item.planScore +'</td><td>'+item.planTime +'</td><td>'+item.overScore +'</td><td> <button type="button" data-row='+(JSON.stringify(item))+'  class="btn btn-primary-oe btn-sm">选择</button> </td></tr>'
        });

        htmlStr += '<div class="search-result-container"> ' +
            '<table class="table table-bordered table-theme-default table-vertical-middle table-small"> ' +
            '<thead> ' +
   ...
   $(function(){
       ...
       $modalBtnSearch.lessonCodeSearch({
           ...
           noResultTemplate: '<div class="search-result-container text-center" style="border-top: 1px solid #ddd">没有查询结果，请重新填入搜索条件</div>',
